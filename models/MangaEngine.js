...
class MangaEngine {
  static async searchManga(searchTerm) {
    const mangaList = await fetch('https://mangalivre.net/lib/search/series.json', {
      headers: {
  ...
  static async searchMangaVolumes(mangaId, page) {
    const volumeList = await fetch(`https://mangalivre.net/series/chapters_list.json?page=${page}&id_serie=${mangaId}`, {
      headers: {
